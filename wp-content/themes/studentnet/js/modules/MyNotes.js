...
class MyNotes {
  constructor() {
    if (document.querySelector("#my-notes")) {
      axios.defaults.headers.common["X-WP-Nonce"] = studentnetData.nonce
  ...
  async deleteNote(e) {
    ...
    try {
      const response = await axios.delete(studentnetData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"))
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios.post(studentnetData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost)
      this.makeNoteReadOnly(thisNote)
    } catch (e) {
      console.log("Sorry")
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios.post(studentnetData.root_url + "/wp-json/wp/v2/note/", ourNewPost)
      console.log(response.data);

      if (response.data != "You have reach your note limit.") {
